      ...
      dataTableConfiguration: {
        ...
        */
        ...
        language: {
              ...
              "search": "_INPUT_",
              "searchPlaceholder": @if (isset($isSubmitCasePath) && $isSubmitCasePath) "{{ trans('flexi.search_only_coloums') }}" @else "{{ trans('backpack::crud.search') }}..." @endif,
  ...
  <script type="text/javascript">
    jQuery(document).ready(function($) {
      ...
      @endif
      @if (isset($isSubmitCasePath) && $isSubmitCasePath)
        var timer, value;
        $('#crudTable_filter input[type="search"]')
            .off()
            .on('keyup change search', async function (e) {
                await clearTimeout(timer);
                if (value != this.value && (e.keyCode == 13 || this.value == "" || e.type == 'change')) {
                    value = this.value;
                    crud.table.search(this.value, true, false).draw();
                } else {
                    timer = await setTimeout(() => $(this).trigger('change'), 2000);
                }
            });
      @endif
